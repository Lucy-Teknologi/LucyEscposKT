...
import app.lucys.lib.lucyescposkt.android.escpos.image
...
class MainViewModel @Inject constructor(
    ...
    private val _messages = MutableStateFlow(emptyList<EPStreamData>())
    ...
    fun connect(device: PrinterModel) {
        ...
        viewModelScope.launch {
            ...
            connection.connect()
            val res = connection.send(command, 10.seconds)
            Log.d("MainViewModel", res.toString())
            connection.disconnect()
        }
    }

    fun stream(device: PrinterModel) {
